...
from ursina import *
from rubik_solver import solve_rubik_cube, RubikEnv
...
class Game(Ursina):
    ...
    def toggle_game_mode(self):
        ...
        if self.action_mode:
            self.message.text = 'ACTION mode ON (to switch - press middle mouse button)'
        else:
            self.message.text = 'VIEW mode ON (to switch - press middle mouse button)'
            self.solve_cube_with_ai()

    def toggle_animation_trigger(self):
        self.action_trigger = not self.action_trigger

    def solve_cube_with_ai(self):
        state = solve_rubik_cube(self.env)
        self.apply_solution_to_cube(state)

    def apply_solution_to_cube(self, state):
        pass
